...
def evaluate_metric(metric, stem, remove_stop, with_ref, prompt='overall'):
    ...
    if with_ref:
        ranks_file_path = os.path.join('outputs',
                                       'wref_{}{}{}_{}_rank_correlation.csv'.format(metric, stemmed_str, stop_str,
                                                                                    prompt))
    else:
        ranks_file_path = os.path.join('outputs',
                                       'woref_{}{}{}_{}_rank_correlation.csv'.format(metric, stemmed_str, stop_str,
                                                                                     prompt))
    ...
    if metric.startswith('infersent'):
        ...
        pass
        if 'human' in metric:
            rewarder = Rewarder(os.path.join(MODEL_WEIGHT_DIR,
                                             'pcc0.3528_epoch50_batch32_pairwise_trainPercent0.64_lrate0.0003_linear.model'))
        elif 'sts' in metric:
        ...
        print(
            'Make sure that your have started the mover server. Find details at https://github.com/AIPHES/emnlp19-moverscore.')
    ...
    for i, (article_id, scores_list) in tqdm(enumerate(sorted_scores.items())):
        ...
        if stem and remove_stop:
            sys_summs = [" ".join(sent2stokens_wostop(s['sys_summ'], stemmer, stopwords_list, 'english', True)) for s in
                         scores_list]
            ...
            sys_summs = [" ".join(sent2tokens_wostop(s['sys_summ'], stopwords_list, 'english', True)) for s in
                         scores_list]
        ...
        if 'rouge' in metric.lower():
            ...
            if with_ref:
                auto_metric_ranks = [bleu(ss, [ref_summ], n, smooth=False) for ss in sys_summs]
            else:
                auto_metric_ranks = [bleu(ss, [article], n, smooth=False) for ss in sys_summs]
        elif metric.startswith('meteor'):
            if with_ref:
                auto_metric_ranks = [meteor(ss, [ref_summ]) for ss in sys_summs]
            else:
            ...
            if with_ref:
                auto_metric_ranks = [s2v.score(ss, ref_summ) for ss in sys_summs]
            else:
                auto_metric_ranks = [s2v.score(ss, article) for ss in sys_summs]
        elif metric.startswith('bert'):
            if 'human' in metric:
                if with_ref:
                    auto_metric_ranks = [rewarder(ref_summ, ss) for ss in sys_summs]
                else:
                    auto_metric_ranks = [rewarder(article, ss) for ss in sys_summs]
            elif 'sts' in metric or 'nli' in metric:
                if with_ref:
                    auto_metric_ranks = [sts_bert_rewarder(bert_model, ss, ref_summ) for ss in sys_summs]
                else:
                    auto_metric_ranks = [sts_bert_rewarder(bert_model, ss, article) for ss in sys_summs]
            else:  # raw BERT encoder
                if with_ref:
                    auto_metric_ranks = [raw_bert_rewarder(bert_model, bert_tokenizer, ss, ref_summ) for ss in
                                         sys_summs]
                else:
                    auto_metric_ranks = [raw_bert_rewarder(bert_model, bert_tokenizer, ss, article) for ss in sys_summs]
        elif metric.startswith('mover'):
            if '1' in metric:
                mm = 'wmd_1'
            elif '2' in metric:
                ...
                mm = 'smd'
            if with_ref:
                cases = [[[ss], [ref_summ], mm] for ss in sys_summs]
            else:
...
def parse_args():
    ...
    ap.add_argument('-m', '--metric', type=str, default='mover-1',
                    choices=['ROUGE-1-F', 'ROUGE-1-R', 'ROUGE-2-F', 'ROUGE-2-R', 'ROUGE-L-F', 'ROUGE-L-R',
                             'ROUGE-SU*-F',
                             'ROUGE-SU*-R', 'bleu-1', 'bleu-2', 'bleu-3', 'bleu-4', 'bleu-5', 'meteor',
                             'infersent', 'bert-raw', 'bert-sts', 'bert-nli', 'bert-human', 'mover-1', 'mover-2',
                             'mover-smd'], help='compare which metric against the human judgements')
    ap.add_argument('-p', '--prompt', type=str, default='overall',
                    help='which human ratings you want to use as ground truth', choices=['overall', 'grammar'])
    ap.add_argument('-r', '--with_ref', type=int, default=0,
                    help='whether to use references in your metric; 1: yes, 0: no')
    ap.add_argument('-s', '--stem', type=int, help='whether stem the texts before computing the metrics; 1 yes, 0 no')
    ap.add_argument('-rs', '--remove_stop', type=int,
                    help='whether remove stop words in texts before computing the metrics; 1 yes, 0 no')
    args = ap.parse_args()
