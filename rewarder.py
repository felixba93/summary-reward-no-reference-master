...
class Rewarder():
    def __init__(self, weight_path, model_type='linear', vec_dim=1024, device='gpu'):
        ...
        self.reward_model = build_model(model_type,
                                        vec_dim * 2)  # times 2 because the input to the model is the concatenation of the doc-vec and the summ-vec
    ...
    def __call__(self, doc, summ):
        doc_vec = list(np.zeros(1024))#raw_bert_encoder(self.bert_model, self.bert_tokenizer, [doc], gpu=self.gpu))  # np.zeros(1024))
